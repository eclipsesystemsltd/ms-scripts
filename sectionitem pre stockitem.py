...
def savePart(cur, part, variantstring, inassembly, assemblylevel, lineNo, sectionId, partId):
	try:
		query = 'insert into sectionitem(ref,            partnumber,      altpartnumber,      altManuPartnumber,      altManuName,      description,      quantity,      referenceonly,      optional,      variantstring, isassembly,      inassembly, assemblylevel, line,   section_id, part_id) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
		values = (                       part.refNumber, part.partNumber, part.altPartNumber, part.altManuPartNumber, part.altManuName, part.description, part.quantity, part.referenceonly, part.optional, variantstring, part.isAssembly, inassembly, assemblylevel, lineNo, sectionId,  partId)
...
def findPart(cur, part):
	try:
		query = 'select p.id from part p where p.part_number=%s'
		values = (part)
		cur.execute(query, values)
		id = [r[0] for r in cur.fetchall()]
		return id[0]
